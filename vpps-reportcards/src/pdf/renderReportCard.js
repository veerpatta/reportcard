...
async function drawFrontPage(ctx, student, barImage) {
    ...
    // ── 4. Summary tiles ──
    y = drawSummaryTiles(page, font, fontBold, computed, info, y);
    y -= 12;
...
async function drawBackPage(ctx, student, barImage, radarImage) {
    ...
    // ── Charts & Grading (right half) ──
    const chartAreaH = y - tableBottom + 40;

    // Expand Radar Chart
    const radarBottom = embedChartImage(page, radarImage, chartX, y, chartW, chartAreaH * 0.52,
        'Performance Radar', fontBold);

    // ── Component Breakdown Table (Bottom Right) ──
    const bkdTableY = radarBottom - 18;
    const breakdownStartY = drawComponentBreakdown(page, font, fontBold, marks, drawSubjects, chartX, bkdTableY, chartW);

    // ── Grading Scale (Bottom Right) ──
    const scaleY = breakdownStartY - 18;
    drawGradingScale(page, font, fontBold, chartX, scaleY, chartW);
...
function drawHeader(page, fontBold, font, logoImage, y) {
    ...
    const tagline = 'Affiliated to Rajasthan Education Board';
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
function drawSummaryTiles(page, font, fontBold, computed, info, y) {
...
/* ══════════════════════════════════════════════════════════════
   HELPER — drawGradingScale()
   ══════════════════════════════════════════════════════════════ */
function drawGradingScale(page, font, fontBold, startX, startY, width) {
    ...
    let y = startY - 10;
    const rowH = 12;
    const col1W = width * 0.45;
    const col2W = width * 0.55;
    ...
    page.drawText('Marks Range', { x: startX + 4, y: y - rowH + 4, size: 6, font: fontBold, color: C.white });
    page.drawText('Grade', { x: startX + col1W + 4, y: y - rowH + 4, size: 6, font: fontBold, color: C.white });
    ...
    grades.forEach((g, i) => {
        ...
        page.drawText(g[0], { x: startX + 4, y: y - rowH + 4, size: 6, font, color: C.text });
        page.drawText(g[1], { x: startX + col1W + 4, y: y - rowH + 4, size: 6, font: fontBold, color: C.text });
